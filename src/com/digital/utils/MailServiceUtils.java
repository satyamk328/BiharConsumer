...
public interface MailServiceUtils {
	...
	public static String generateBookingTicketMail(TicketVO ticketVO) {
		...
		String body = BODY.replace("${CUSTOMERNAME}", ticketVO.getEmail().split("@")[0])
				.replace("${TICKETID}", ticketVO.getPnr()).replace("${TRAVELNAME}", ticketVO.getTravelName())
	...
	public static String CONFIRMTICKETSMS = "PNR:${PNR},TRVLN:${TRAVELNAME},DOJ:${DOJ},${SEATTYPE},${SOURCE}-${DESTNAME},DP:${DESPTIME},${CUSTNAME} ${ISLOWER} ${SEATNAME}, Fare:${FARE}";

	public static String confirmBookingSMS(TicketVO ticketVO) {
		List<String> seatNo = ticketVO.getSeatDataToOperate().stream().map(e -> e.getSeatNumber())
				.collect(Collectors.toList());
		return CONFIRMTICKETSMS.replace("${PNR}", ticketVO.getPnr()).replace("${TRAVELNAME}", ticketVO.getTravelName())
				.replace("${DOJ}", ticketVO.getDepartureDate())
